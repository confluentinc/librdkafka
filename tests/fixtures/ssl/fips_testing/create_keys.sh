...
for INTERMEDIATE in true false; do
    ...
    # Re-export keystore with 3DES encryption
    echo "# $CN: Re-export keystore with 3DES encryption"
    TEMP_KEYSTORE="${KEYSTORE}.tmp"
    TEMP_PEM="${KEYSTORE}.pem"
    mv "$KEYSTORE" "$TEMP_KEYSTORE"

    # Extract certificate and key to PEM format
    openssl pkcs12 -in "$TEMP_KEYSTORE" -out "$TEMP_PEM" \
        -passin "pass:${CN_KEYSTORE_PASSWORD}" \
        -passout "pass:${CN_KEYSTORE_PASSWORD}"

    # Re-export to PKCS12 with 3DES encryption
    openssl pkcs12 -export -in "$TEMP_PEM" -out "$KEYSTORE" \
        -passin "pass:${CN_KEYSTORE_PASSWORD}" \
        -passout "pass:${CN_KEYSTORE_PASSWORD}" \
        -name "$CN" \
        -keypbe PBE-SHA1-3DES -certpbe PBE-SHA1-3DES -macalg sha1

    rm "$TEMP_KEYSTORE" "$TEMP_PEM"

    echo "# $CN: Export PEM certificate"
    openssl pkcs12 -in "$KEYSTORE" -out "$CERTIFICATE" \
        -nokeys -passin "pass:${CN_KEYSTORE_PASSWORD}"

    echo "# $CN: Export PEM key with 3DES encryption"
