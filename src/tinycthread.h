...
#else
  #ifdef __cplusplus
    /* In C++, avoid macro that conflicts with std::call_once */
    static inline int call_once(once_flag *flag, void (*func)(void)) {
        pthread_once(flag, func);
        return 0; /* pthread_once returns void on some platforms, assume success */
    }
  #else
