 ...
 * In librdkafka release pipeline a static build containing libsasl2
   could be chosen instead of the alternative without it.
   That caused the libsasl2 dependency to be required for some confluent-kafka-go
   platforms, including v2.1.0-linux-musl-arm64 and v2.3.0-linux-musl-arm64.
   Solved by correctly excluding the build configured with that library,
   when targeting a static build.
   Happening since v2.0.2, for some platforms, when using static binaries (#4666).
