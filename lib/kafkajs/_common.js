...
function createReplacementErrorMessage(cOrP, fnCall, property, propertyVal, replacementVal, isLK = false) {
  if (isLK) {
    replacementVal = `rdKafka: { ${replacementVal}, ... }`
  }
...
const CompatibilityErrorMessages = Object.freeze({
  ...
  sslObject: () =>
    "The 'ssl' property must be a boolean. Any additional configuration must be provided through the 'rdKafka' property.\n" +
    "Before: \n" +
...
function checkAllowedKeys(allowedKeysSpecific, config) {
  ...
  for (const key of Object.keys(config)) {
    if (!allowedKeysCommon.includes(key) && !allowedKeysSpecific.includes(key)) {
      throw new error.KafkaJSError(CompatibilityErrorMessages.unsupportedKey(key), {
        code: error.ErrorCodes.ERR__INVALID_ARG,
      });
    }
  }
