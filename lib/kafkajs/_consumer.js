...
class Consumer {
  ...
  async #finalizedConfig() {
    ...
    checkAllowedKeys(allowedKeysSpecific, this.#kJSConfig);
    ...
    if (Object.hasOwn(this.#kJSConfig, 'partitionAssignors')) {
      ...
      globalConfig['partition.assignment.strategy'] = PartitionAssigners.roundRobin;
    }

    if (Object.hasOwn(this.#kJSConfig, 'sessionTimeout')) {
      globalConfig['session.timeout.ms'] = this.#kJSConfig.sessionTimeout;
    } else {
      globalConfig['session.timeout.ms'] = 30000;
    }
    ...
    if (Object.hasOwn(this.#kJSConfig, 'allowAutoTopicCreation')) {
      ...
      globalConfig['allow.auto.create.topics'] = true;
    }
    ...
    if (Object.hasOwn(this.#kJSConfig, 'maxBytes')) {
      ...
      globalConfig['fetch.message.max.bytes'] = 10485760;
    }
    ...
    if (Object.hasOwn(this.#kJSConfig, 'fromBeginning')) {
      topicConfig['auto.offset.reset'] = this.#kJSConfig.fromBeginning ? 'earliest' : 'latest';
    }

    if (Object.hasOwn(this.#kJSConfig, 'autoCommit')) {
      globalConfig['enable.auto.commit'] = this.#kJSConfig.autoCommit;
    }

    if (Object.hasOwn(this.#kJSConfig, 'autoCommitInterval')) {
      globalConfig['auto.commit.interval.ms'] = this.#kJSConfig.autoCommitInterval;
    }

    if (Object.hasOwn(this.#kJSConfig, 'autoCommitThreshold')) {
    ...
    this.#logger.setLogLevel(severityToLogLevel[globalConfig.log_level]);
