...
const { kafkaJSToRdKafkaConfig,
  ...
  severityToLogLevel,
  checkAllowedKeys,
  CompatibilityErrorMessages } = require('./_common');
...
class Producer {
  ...
  async #finalizedConfig() {
    ...
    globalConfig['partitioner'] = 'murmur2_random';
    ...
    if (Object.hasOwn(this.#kJSConfig, 'compression')) {
      globalConfig['compression.codec'] = this.#kJSConfig.compression;
    }

    if (Object.hasOwn(this.#kJSConfig, 'acks')) {
      globalConfig['acks'] = this.#kJSConfig.acks;
    }

    if (Object.hasOwn(this.#kJSConfig, 'timeout')) {
      globalConfig['request.timeout.ms'] = this.#kJSConfig.timeout;
    }
