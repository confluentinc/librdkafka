...
class MessageCache {
    ...
    addMessages(messages) {
        ...
        // const ppcObj = this.ppcList.map(ppc => ppc.cache.map(c => ({topic: c.topic, partition: c.partition, offset: c.offset})));
        // console.log("addMessages", "\ntpToPPC: ", this.tpToPpc, "\nppcList: ", JSON.stringify(ppcObj, null, 2));
    }
    ...
    return(idx) {
        if (!this.pendingIndices.has(idx)) {
             ...
             * returning to us. */
            console.error("Returning unowned index", idx, "to cache. Discarding it.");
        } else {
     ...
     */
    next(idx = -1) {
        // console.log("next called with ", idx, " and pending indices", this.pendingIndices, " and ppcList ", JSON.stringify(this.ppcList.map(ppc => ({
        //     ...ppc,
        //     cache: ppc.cache.map(c => ({topic: c.topic, partition: c.partition, offset: c.offset})),
        // })), null, 2));
        let index = idx;
    ...
    clear() {
        if (this.pendingSize() > 0) {
            console.error('clear: pendingIndices = ', this.pendingIndices, console.trace());
