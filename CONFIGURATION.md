...
enforce.required.acks                    |         false | Fail messages locally if the currently known ISR count for a partition is less than `request.required.acks`. `request.required.acks` must be greater than zero for this configuration to have effect. **NOTE**: The ISR count is fetched from the broker at regular intervals (`topic.metadata.refresh.interval.ms`) and might thus be outdated.
