 ...
 */
static int parse_test_conf(rd_kafka_conf_t *conf, char *topic_prefix, size_t prefix_size) {
    FILE *fp;
    char line[512];
    char *key, *val, *ptr;
    int found_prefix = 0;
...
static int cleanup_topics(rd_kafka_conf_t *conf, const char *topic_prefix) {
    ...
    if (rd_kafka_metadata(rk, 0, NULL, &metadata, TIMEOUT_MS) != RD_KAFKA_RESP_ERR_NO_ERROR) {
    ...
    for (i = 0; i < metadata->topic_cnt; i++) {
        if (strncmp(metadata->topics[i].topic, topic_prefix, prefix_len) == 0) {
            topic_count++;
        }
    }
    ...
    for (i = 0; i < metadata->topic_cnt && idx < topic_count; i++) {
        if (strncmp(metadata->topics[i].topic, topic_prefix, prefix_len) == 0) {
            del_topics[idx] = rd_kafka_DeleteTopic_new(metadata->topics[i].topic);
            idx++;
        }
    }
    ...
    if (event) {
        ...
        if (result) {
            ...
            size_t result_count;
            topic_results = rd_kafka_DeleteTopics_result_topics(result, &result_count);
            
            for (i = 0; i < (int)result_count; i++) {
                rd_kafka_resp_err_t err = rd_kafka_topic_result_error(topic_results[i]);
                const char *topic_name = rd_kafka_topic_result_name(topic_results[i]);
...
int main() {
    ...
    if (parse_test_conf(conf, topic_prefix, sizeof(topic_prefix)) < 0) {
        if (access("test.conf", R_OK) != 0) {
            printf("No config file found - skipping topic cleanup\n");
        } else {
            printf("No topic prefix configured - skipping topic cleanup\n");
        }
